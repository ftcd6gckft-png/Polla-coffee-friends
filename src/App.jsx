 ...
 *  4. Los datos del fixture se cargaron.
 *  5. El branding Pantone se aplica.
 *
 * Cuando todo aparezca en verde, pasamos a la Entrega 2.
 */
...
function Landing() {
  ...
  useEffect(() => {
    ...
    async function ping() {
      ...
      try {
        // Diagnóstico: leer un documento que probablemente no existe.
        // Si Firestore está bien configurado, getDoc devuelve un snapshot
        // con .exists() = false sin lanzar error.
        // Si las reglas bloquean (permission-denied) o no hay base de datos,
        // lanzará un error que capturamos abajo.
        const probeRef = doc(db, 'diagnostic', 'probe');
